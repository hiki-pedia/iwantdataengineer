...
현재는 Phase 1을 완료하고 Phase 2 자동 수집을 운영하고 있다. Phase 5 데이터 품질과 재처리를 진행하던 중, 이후 모델·RAG·리포트 결과가 들어갈 화면과 API 계약을 먼저 고정하기 위해 Phase 3 Electron Dashboard를 완료하고 Phase 4 FastAPI 핵심 조회 계층까지 구현했다. 이제 다시 Phase 5의 남은 운영 검증으로 돌아간다. 이 글은 프로젝트가 끝난 뒤 작성하는 결과 보고서가 아니라, 각 Phase에서 세운 목표와 구현 내용, 해결한 문제, 아직 남은 과제를 계속 추가하는 개발 기록이다.
...
| 구현 범위 | Phase 1·3 완료, Phase 2 운영 중, Phase 4 핵심 연결 구현, Phase 5 진행 중 |
| 현재 작업 | Phase 3 문서화 후 데이터 품질 운영 검증 재개 |
...
| 5 | 데이터 품질과 재처리 | Coverage, 누락, 중복, 날짜 범위 검사와 백필·재처리 절차 구현 | 진행 중 |
...
현재 이 기록은 Phase 1, 운영 중인 Phase 2, 진행 중인 Phase 5 품질 계층과 함께 Phase 3 Electron 및 Phase 4 핵심 API 연결 결과까지 다룬다. 모델, RAG, AWS 항목은 아직 계획 상태로 구분한다.
...
현재 Phase 5는 진행 중이다. 품질 검사, 거래소 캘린더, pykrx_naver 보조 수집, curated 생성, 재처리와 Airflow Task 연결까지 구현했다. 독립 KRX 원시 시세는 계정 인증이 필요해 아직 최종 교차 검증하지 못했다.
...
| 실제 개장 기록은 있으나 보조 공급자 값 없음 | 2007-03-02 |
...
PostgreSQL에는 `data_quality_issues` 테이블을 추가했다. 종목, 관찰일, 공급자, 문제 코드, 심각도, 상태와 해결 방법을 기록한다. OHLC 교체와 5개 거래일 보완은 `resolved`, SK하이닉스 보정주가와 `2007-03-02` 누락은 `open` 상태다.
...
| 미해결 | SK하이닉스 보정주가 778행, 2007-03-02 데이터 |
...
Phase 5를 아직 완료로 처리하지 않는다. 남은 작업은 KRX 계정 인증을 설정해 원시 시세로 다시 검증하고, 새 Airflow Task 체인의 첫 정기 실행과 의도적인 실패·재시도를 확인하는 것이다. 수동 DAG Run에서는 세 Task가 모두 성공했다.
...
데이터 검증 화면은 시장별 요약뿐 아니라 종목, 심각도, 검사 코드, 설명, 영향 행 수와 검사 기준일을 표시한다. 현재 실제 리포트에서는 오류 0건, 경고 12건, 영향 종목 11개를 조회하며, SK하이닉스의 수정주가 이상 778행과 한국장 `2007-03-02` 예상 거래일 누락을 화면에서 확인할 수 있다.
...
- pykrx_naver는 보조 공급자이며 KRX 인증 기반 원시 시세 검증은 아직 남아 있다.
...
- 강제 실패, 네트워크 단절, DB 중단 상황의 재시도·복구 검증이 남아 있다.
...
Phase 1에서는 독립 실행 가능한 ETL과 저장 계층을 구현했고, Phase 2에서는 이를 Airflow로 자동화했다. Phase 3에서는 Electron의 정보 구조와 실제 데이터 조회 화면을 완성했고, Phase 4의 핵심 FastAPI 경계를 함께 검증했다. 이제 Phase 5로 돌아가 실제 공급자 이상, 원천 보존, 교차 검증, curated 계층과 품질 Gate의 남은 운영 검증을 계속한다.
