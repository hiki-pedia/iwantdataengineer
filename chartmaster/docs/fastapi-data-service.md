...
대시보드 응답은 자산 최신값, 시장별 품질 요약, 종목별 품질 이슈, 파이프라인 표시 상태, 급등락 후보, 예측 상태와 리포트 상태를 포함한다. 품질 이슈에는 심각도, 검사 코드, 설명, 영향 행 수와 검사 기준일이 들어간다. 가격 API는 서버2의 `processed/features` CSV에서 OHLCV와 이동평균을 반환한다.
